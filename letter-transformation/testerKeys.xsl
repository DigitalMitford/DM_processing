...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable as="element(tei:p)" name="input">
        <tei:p>Stuff to look up, like <tei:persName ref="#MRM">Mary Russell
            Mitford</tei:persName>.</tei:p>
    </xsl:variable>
    <xsl:key name="namedEntityById" match="tei:person | tei:org" use="@xml:id"/>
    ...
    <xsl:template match="tei:persName">
        <span class="context" title="person">
            ...
            <span class="note">
                ...
                <xsl:value-of select="key('namedEntityById', substring-after(current()/@ref, '#'), $si)"/>
            </span>
        </span>
    </xsl:template>
    <xsl:template match="/">
