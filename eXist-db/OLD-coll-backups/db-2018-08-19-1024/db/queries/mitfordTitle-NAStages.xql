...
let $mitfordColl:=collection('/db/mitford/')/*[.//titleStmt/title[1][not(contains(., "Title"))]]

let $titles := ($mitfordColl//text//p//title[not(ancestor::note[not(@resp='MRM')])]/@ref/string(), $mitfordColl//text//p//bibl[not(ancestor::note)]/@corresp/string())
...
let $fileMatch := $mitfordColl[.//text//p//*/@*[substring-after(string(), '#') = $n1]]    
for $f in $fileMatch
let $fileTitle := $f//titleStmt/title[1]/normalize-space(string())

(: 2016-07-10 ebb: we're screening these out now :let $SIeditorScreen :=
    if ($f//text//p//*[contains(@*, $n1)][ancestor::note[not(contains(@resp, "MRM"))]]) then "EDITOR-NOTE"
    else if ($f//text//p//*[contains(@*, $n1)][ancestor::note[contains(@resp, "MRM")]]) then "MRM-NOTE"
    else "MAIN_TEXT"  :)

let $otherTitles := ($f//text//p//title[not(contains(@ref, $n1))][not(ancestor::note[not(@resp='MRM')])]/@ref/string(), $f//text//bibl[not(contains(@corresp, $n1))][not(ancestor::note)]/@corresp/string())
...
return concat($n1, "&#x9;", $SIcondition, "&#x9;", $SIcat, "&#x9;", $SIdate, "&#x9;", $fileTitle, "&#x9;", $n2, "&#x9;", $SIcondition_n2, "&#x9;", $SIcat_n2, "&#x9;", $SIdate_n2), "&#10;")
;

let $filename := "MRMTitleNet.tsv"
let $doc-db-uri := xmldb:store("/db/output", $filename, $ThisFileContent, "text/plain")
return $doc-db-uri
(: output at :http://dxcvm10.psc.edu:8080/exist/rest/db/output/MRMTitleNet.tsv ) :) 