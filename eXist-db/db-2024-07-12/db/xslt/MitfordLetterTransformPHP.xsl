...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dm="http://digitalmitford.org/nss/dm" version="3.0" xpath-default-namespace="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="dm">
            ...
            <div id="letterHead">
                ...
                <p class="boilerplate">
                    <span>
                        ...
                        psu.edu) <a rel="license" href="http://creativecommons.org/licenses/by-nc-sa/4.0/">
  ...
  <!-- 2021-08-15 ebb: This template processes a series of statements that output links to the image files (that we can't open yet)
    ...
    <xsl:template match="note[not(ancestor::*[@sortKey])]">
        ...
        <span id="Note{count (preceding::note) + 1}" class="anchor">[<xsl:value-of select="count (preceding::note)+ 1"/>] <span class="note" id="n{count (preceding::note) + 1}">
                <xsl:if test="@resp"> <xsl:apply-templates/>
                    <xsl:text>—</xsl:text>
                ...
                <span class="si">
                    ...
                    <xsl:choose>
                        <xsl:when test="$siPers//forename">  <xsl:value-of select="$si//*[@xml:id = substring-after(current()/@ref, '#')]//forename[1]"/>
                            ...
                            <xsl:if test="count($siPers//surname) gt 1"> <xsl:value-of select="string-join($siPers//surname[2] | surname[@type='maiden'], ' ')"/>
                                <xsl:text> </xsl:text>
                            </xsl:if>
        ...
        <span class="context" title="title">
            ...
            <xsl:if test="($si//*[@xml:id = substring-after(current()/@ref, '#')] | $si//*[@xml:id = substring-after(current()/@corresp, '#')]) and not(ancestor::note)"> <span class="si">
                ...
                <xsl:if test="$siBibl/note">
                    <xsl:for-each select="$siBibl/note"> <br/>
                            <xsl:value-of select="current()"/>
                        <xsl:if test="@resp">
                            <xsl:text>—</xsl:text>
                            <xsl:sequence select="dm:respHandler(@resp)"/>
    ...
    <xsl:template match="gap | del[not(text())]">
        <span class="damage">
            ...
            <xsl:if test="@quantity">
                ...
                <xsl:if test="number(@quantity) gt 1 and not(matches(@unit, 's$'))">
                    <xsl:text>s</xsl:text>
                </xsl:if>
            </xsl:if>
            <xsl:if test="@quantity and @reason">
                <xsl:text>, </xsl:text>
            </xsl:if>
            <xsl:if test="@reason">
    ...
    <xsl:template match="add">
        <xsl:choose> <xsl:when test="metamark">
                ...
                <span class="add {@hand ! tokenize(., '#')[last()]}">
                    <xsl:apply-templates select="metamark/following-sibling::*|text()"/>
                </span>
        </xsl:when>
            <xsl:otherwise>
                <span class="add {@hand ! tokenize(., '#')[last()]}"> <xsl:apply-templates/>
                </span>
            </xsl:otherwise>
