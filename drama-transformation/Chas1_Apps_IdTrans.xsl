...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"    xpath-default-namespace="http://www.tei-c.org/ns/1.0"
   ...
   * I am worried about how I'm processing the tvar parameter. 
   * In the latest iteration, elements preceding apps are being trimmed out but not ported in, so something's up with some params tunneling. -->
...
<xsl:variable name="currWit">
       <xsl:value-of select="$tVar"/><xsl:value-of select="rdg"/>
</xsl:variable>
                       <rdg wit="{rdg/@wit}">
 ...
 <!--Test 3: There is an element (that's neither a note nor an app) immediately preceding the singleton app. -->    
          <xsl:when test="preceding-sibling::*[1][not(name()='note') and not(name()='app')] and  matches(preceding-sibling::text()[1], '^\s*$') and not(rdg/pb[matches(preceding-sibling::text()[1], '^\s*$')])">    
             <app>
                 <rdg wit="#{$unrepWit}"><xsl:value-of select="$tVar"/></rdg>    
         ...
         <app>
              <rdg wit="#{$unrepWit}">
                  ...
                  <xsl:variable name="currWit">
                      <xsl:value-of select="rdg"/><xsl:value-of select="$AfterVar"/>
                  </xsl:variable>
  ...
  <!--Test 5: There's a first following sibling element but no text after the singleton app. This should only fire in the event of no preceding text or element. -->           
             ...
             <xsl:when test="rdg/pb[matches(preceding-sibling::text()[1], '^\s*$') and matches(following-sibling::text()[1], '^\s*$')]">
                 <pb edRef="{rdg/@wit}" n="{rdg/pb/@n}"/>                 
             </xsl:when>
     
         <!--<xsl:otherwise>
               <xsl:comment>BLAAARRGH</xsl:comment> 
             <xsl:comment>I am tVar: <xsl:value-of select="$tVar"/></xsl:comment>
             <app>
...
<!-- Here we process an element (not a note or an app) with no text but white space between it and the singleton app. 
    NOTE: Changing this so we turn it into a tunneling param-->      
    <xsl:template match="body//*[not(name()='note') and not(name()='app') and matches(following-sibling::text()[1], '^\s*$') and following-sibling::*[1][name()='app'][count(rdg) = 1]]">
        <xsl:variable name="precElem" select="normalize-space(.)"/>
        <xsl:apply-templates select="following-sibling::app[1]">
           <xsl:with-param name="tVar" select="$precElem" tunnel="yes"/> 
        </xsl:apply-templates>
    </xsl:template>
    
    <!--This is the same as the above except it matches an element (not an app or note) and looks on the preceding-sibling:: axis and the first element it finds is a singleton app.    -->
    <xsl:template match="body//*[not(name()='note') and not(name()='app') and matches(preceding-sibling::text()[1], '^\s*$') and preceding-sibling::*[1][name()='app'][count(rdg) = 1]]">
        <xsl:variable name="followElem" select="normalize-space(.)"/>
        <xsl:apply-templates select="preceding-sibling::app[1]">
            <xsl:with-param name="AfterVar" select="$followElem" tunnel="yes"/> 
        </xsl:apply-templates>
